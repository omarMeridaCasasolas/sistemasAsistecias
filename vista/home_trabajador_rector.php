<?php
    ...
    if(isset($_SESSION["idTrabajador"])){
        $listaFunciones = $_SESSION['listaFunciones'];
        //echo count($listaFunciones);
        //var_dump($listaFunciones[1]['titulo_tarea']);
    }else{
...
<head>
    ...
    <title>Bienvenido personal de trabajo</title>
...
<header>
    <nav class="navbar navbar-expand-sm bg-dark navbar-dark">
        <h2 class="text-white p-2"><?php echo $_SESSION['cargoTrabajador'].": ".$_SESSION['nombreTrabajador'];?></h2>
        <div class="d-block">
        <a class="float-right" href="../controlador/formCerrarSession.php">Cerrar session</a>
        </div>
        <?php 
            for ($i = 0; $i < count($listaFunciones); $i++) { 
                $tmp = $listaFunciones[$i]['titulo_tarea'];
                if("Editar sus datos personales" == $tmp){
                    echo "<button class='btn btn-warning'>Editar mi informacion</button>";
                    break;
                }
            }
        // if(in_array("Editar sus datos personales",$listaFunciones)){
        //         echo "<button class='btn btn-warning'>Editar mi informacion</button>";
        //     }
        ?>
    </nav>
...
<body class="bg-secondary">
    <main class="container bg-white p-2">
        ...
        <?php
            for ($i = 0; $i < count($listaFunciones); $i++) { 
                $tmp = $listaFunciones[$i]['titulo_tarea'];
                if("Crear facultad o unidad" == $tmp){
                    echo '<button type="button" class="btn btn-primary justify-content-end" data-toggle="modal" data-target="#myModal2"> Crear facultad
                    </button>';
                    break;
                }
            }
