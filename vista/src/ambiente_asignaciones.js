$(document).ready(function() {
    ...
    listarTablaMateriasDepartamento = $("#tablaMateriasDep").DataTable({
        ...
        "columns":[
            {"data":"codigo_materia"},
            {"data":"nombre_materia"},
            {"defaultContent": "<div class='text-center'><div class='btn-group'><button class='btn btn-primary btn-sm btnEditarMateria'><i class='far fa-edit'></i></i></button><button class='btn btn-danger btn-sm btnEliminarMateria'><i class='fas fa-trash-alt'></i></button></div></div>"}
        ]
    });
    ...
    $(document).on("click", "#btnAsignarDocente", function(e){   
        e.preventDefault();//evita el comportambiento normal del submit, es decir, recarga total de la página
        let id_docente = $("#selectDocentes option:selected").val();
        let nom_docente = $("#selectDocentes option:selected").text();    
        if(!existeDentroContenedor("rowDocente/"+id_docente, "contenedorDocentesAsignados")){
            let divRow = document.createElement("div");
            divRow.className = "row";
            divRow.id = "rowDocente/"+id_docente;
            let div1 = document.createElement("div");
            div1.className = "col-md-10";
            let input = document.createElement("input");
            input.className = "form-control"
            input.disabled = "true";
            input.value = nom_docente;
            ...
            button.className = "btn btn-primary quitarAsignacion"
            button.id = "btnDocente/"+id_docente;
            button.innerHTML = "<i class='fas fa-minus'></i>";

            div2.appendChild(button);
            divRow.appendChild(div2);

            let elemDiv = document.getElementById("contenedorDocentesAsignados");
            elemDiv.appendChild(divRow);
            elemDiv.appendChild(document.createElement("br"));
        }   
    });

    //AsignarAuxiliarMateria
    $(document).on("click", "#btnAsignarAuxiliar", function(e){   
        e.preventDefault();//evita el comportambiento normal del submit, es decir, recarga total de la página
        let id_auxiliar = $("#selectAuxiliares option:selected").val();
        let nom_auxiliar = $("#selectAuxiliares option:selected").text();    
        if(!existeDentroContenedor("rowAuxiliar/"+id_auxiliar, "contenedorAuxiliaresAsignados")){
            let divRow = document.createElement("div");
            divRow.className = "row";
            divRow.id = "rowAuxiliar/"+id_auxiliar;
            let div1 = document.createElement("div");
            div1.className = "col-md-10";
            let input = document.createElement("input");
            input.className = "form-control"
            input.disabled = "true";
            input.value = nom_auxiliar;
            ...
            let button = document.createElement("button");
            button.className = "btn btn-primary quitarAsignacion"
            button.id = "btnAuxiliar/"+id_auxiliar;
            button.innerHTML = "<i class='fas fa-minus'></i>";

            div2.appendChild(button);
            divRow.appendChild(div2);

            let elemDiv = document.getElementById("contenedorAuxiliaresAsignados");
            elemDiv.appendChild(divRow);
            elemDiv.appendChild(document.createElement("br"));
        }   
    });
    ...
    $(document).on("click", "#btnAsignarCarrera", function(e){   
        e.preventDefault();//evita el comportambiento normal del submit, es decir, recarga total de la página
        let id_carrera = $("#selectCarreras option:selected").val();
        let nom_carrera = $("#selectCarreras option:selected").text();    
        if(!existeDentroContenedor("rowCarrera/"+id_carrera, "contenedorCarrerasAsignadas")){
            let divRow = document.createElement("div");
            divRow.className = "row";
            divRow.id = "rowCarrera/"+id_carrera;
            let div1 = document.createElement("div");
            div1.className = "col-md-10";
            let input = document.createElement("input");
            input.className = "form-control"
            input.disabled = "true";
            input.value = nom_carrera;
            ...
            let button = document.createElement("button");
            button.className = "btn btn-primary quitarAsignacion"
            button.id = "btnCarrera/"+id_carrera;
            button.innerHTML = "<i class='fas fa-minus'></i>";

            div2.appendChild(button);
            divRow.appendChild(div2);

            let elemDiv = document.getElementById("contenedorCarrerasAsignadas");
            elemDiv.appendChild(divRow);
            elemDiv.appendChild(document.createElement("br"));
        }   
    });
    ...
    //QuitarAsignacion
    $(document).on("click", ".quitarAsignacion", function(e){   
        e.preventDefault();//evita el comportambiento normal del submit, es decir, recarga total de la página
        let array = (this.id).split("/");
        switch(array[0]){
            case "btnDocente":
                quitarDivRow("rowDocente/"+array[1], "contenedorDocentesAsignados");
                break;
            case "btnAuxiliar":
                quitarDivRow("rowAuxiliar/"+array[1], "contenedorAuxiliaresAsignados");
                break;
            case "btnCarrera":
                quitarDivRow("rowCarrera/"+array[1], "contenedorCarrerasAsignadas");
                break;
        }
    });
    ...
    $('#formEditarMateria').submit(function(e){                         
        e.preventDefault(); //evita el comportambiento normal del submit, es decir, recarga total de la página
        let codigo = $("#nomMateriaEdit").attr("name");
        let nomMateria = $.trim($("#nomMateriaEdit").val());
        let nuevo_codigo = $.trim($("#codigoEdit").val());
        let array_docentes = obtenerIdentificadoresDivRow("contenedorDocentesAsignados");
        let array_auxiliares = obtenerIdentificadoresDivRow("contenedorAuxiliaresAsignados");
        let array_carreras = obtenerIdentificadoresDivRow("contenedorCarrerasAsignadas");
        ...
        metodo = 'editarMateria';                       
            ...
            type: "POST",
            datatype:"json",    
            data:  {clase:clase, metodo:metodo, nomMateria:nomMateria, codigo:codigo, nuevo_codigo:nuevo_codigo, array_docentes:array_docentes, array_auxiliares:array_auxiliares, array_carreras:array_carreras},    
...
function cargarContenedoresDivMateria(sis_materia, nom_clase, nom_metodo, nom_divContenedor){
    let elemDiv = document.getElementById(nom_divContenedor);
    borrarContenidoPrevio(elemDiv);
    let datos = {
        clase: nom_clase,
        metodo: nom_metodo,
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            obj.forEach(element => {
                let divRow = document.createElement("div");
                divRow.className = "row";
                if(nom_divContenedor == 'contenedorDocentesAsignados'){
                    divRow.id = "rowDocente/"+element.id_docente;
                }else if(nom_divContenedor == 'contenedorAuxiliaresAsignados'){
                    divRow.id = "rowAuxiliar/"+element.id_aux_docente;
                }else{
                    divRow.id = "rowCarrera/"+element.id_carrera;
                }
                let div1 = document.createElement("div");
                div1.className = "col-md-10";
                let input = document.createElement("input");
                ...
                input.disabled = "true";
                if(nom_divContenedor == 'contenedorDocentesAsignados'){
                    input.value = element.sis_docente+" "+element.nombre_docente;
                }else if(nom_divContenedor == 'contenedorAuxiliaresAsignados'){
                    input.value = element.sis_auxiliar+" "+element.nombre_aux_docente;
                }else {
                    input.value = element.nombre_carrera;
                }
                ...
                button.className = "btn btn-primary quitarAsignacion"
                if(nom_divContenedor == 'contenedorDocentesAsignados'){
                    button.id = "btnDocente/"+element.id_docente;
                }else if(nom_divContenedor == 'contenedorAuxiliaresAsignados'){
                    button.id = "btnAuxiliar/"+element.id_aux_docente;
                }else{
                    button.id = "btnCarrera/"+element.id_carrera;
                }
                button.innerHTML = "<i class='fas fa-minus'></i>";
...
function cargarElemSelectAsignacionesMateria(idElemSelect, id_departamento, nom_clase, nom_metodo){
    let elemSelect = document.getElementById(idElemSelect);
    borrarContenidoPrevio(elemSelect);
    let datos = {
        clase: nom_clase,
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            obj.forEach(element => {
                let option = document.createElement("option");
                if(idElemSelect == 'selectDocentes'){
                    option.value = element.id_docente;
                    option.innerHTML = element.sis_docente+" "+element.nombre_docente;
                }else if(idElemSelect == 'selectAuxiliares'){
                    option.value = element.id_aux_docente;
                    option.innerHTML = element.sis_auxiliar+" "+element.nombre_aux_docente;
                }else{
                    option.value = element.id_carrera;
                    option.innerHTML = element.nombre_carrera;
                }
                elemSelect.appendChild(option);
            });
        },
...
function existeDentroContenedor(idDivRow, idContenedorDivRow){
    let existe = false;
    let contenedor = document.getElementById(idContenedorDivRow);
    let array_nodes = contenedor.childNodes;
    for(let i=0; i<array_nodes.length && !existe; i++){
        let row = array_nodes[i];
        if(row.id == idDivRow){
            existe = true;
        }
    } 
...
function quitarDivRow(idDivRow, idContenedorDivRow){
    let contenedor = document.getElementById(idContenedorDivRow);
    let array_nodes = contenedor.childNodes;
...
function obtenerIdentificadoresDivRow(idContenedorDivRow) {
    let array = new Array();
    let contenedor = document.getElementById(idContenedorDivRow);
    let array_nodes = contenedor.childNodes;
    for(let i=0; i<array_nodes.length; i++){
