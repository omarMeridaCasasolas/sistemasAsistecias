        ...
        if(!is_numeric($templ)){
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['miFilePermiso']) && $_FILES['miFilePermiso']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['miFilePermiso']['tmp_name'])) {
                ...
                try {
                    ...
                    foreach ($listaClases as $x) {
                        $res = $clase->solicitarLicenciaClaseSemURLAux($x,$id,$descripcionLicencia,$enlace);
                        if($res != 1 || $res ==false){
                            header("Location:../vista/home_Auxiliar_docente.php?result=".$res);
                        }  
                    }
                    header("Location:../vista/home_Auxiliar_docente.php?result=success");
                }catch(Exception $e) {
                    echo $e;
                }
            }else{
                foreach ($listaClases as $x) {
                    var_dump($x);
                    $res = $clase->solicitarLicenciaClaseSemAux($x,$id,$descripcionLicencia);
                    if($res != 1 || $res ==false){
                        header("Location:../vista/home_Auxiliar_docente.php?result=".$res);
                        echo "huvo un error\n";
                    }  
            ...
            if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['miFilePermiso']) && $_FILES['miFilePermiso']['error'] == UPLOAD_ERR_OK && is_uploaded_file($_FILES['miFilePermiso']['tmp_name'])) {
                ...
                try {
                    ...
                    echo $enlace."\n";
                    foreach ($listaClases as $x) {
                        $res = $clase->solicitarLicenciaClaseURLAux($x,$descripcionLicencia,$enlace);
                        if($res != 1 || $res ==false){
                            header("Location:../vista/home_Auxiliar_docente.php?result=".$res);
                        }  
                    }
                ...
                foreach ($listaClases as $x) {
                    var_dump($x);
                    $res = $clase->solicitarLicenciaClaseAux($x,$descripcionLicencia);
                    if($res != 1 || $res ==false){
                        header("Location:../vista/home_Auxiliar_docente.php?result=".$res);
                        echo "huvo un error\n";
                    }  
